            ...
            <div class="pa10 bg-warning">
                <form class="form-inline" id="formSearch" action="?"><!-- form 标签开始 -->
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control class_leixing41" data-value="${param.leixing}" id="leixing"
                                name="leixing">
                        ...
                        <script>
                            $(".class_leixing41").val($(".class_leixing41").attr("data-value"))</script>

                    </div>
                    <div class="form-group">
                        申请类型

                        <select class="form-control class_shenqingleixing42" data-value="${param.shenqingleixing}"
                        ...
                        <script>
                            $(".class_shenqingleixing42").val($(".class_shenqingleixing42").attr("data-value"))</script>
                ...
                <table width="100%" border="1" class="table table-list table-bordered table-hover">
                    ...
                    <c:set var="i" value="0"/><c:forEach items="${list}" var="map"><c:set var="i" value="${i+1}"/>
                        <tr id="${map.id}" pid="">
                            ...
                            <td> ${map.mingcheng} </td>
                            <td><ssm:sql var="mapwuzileibie28"
                                         type="find">SELECT leibiemingcheng FROM wuzileibie where id='${map.leixing}'</ssm:sql>${mapwuzileibie28.leibiemingcheng} </td>
                            ...
                            <td> ${map.fafangshijian} </td>
                            <td> ${map.fafangbeizhu} </td>
                            <td> ${map.fafangren} </td>
                            <td align="center">${Info.subStr(map.addtime, 19, "")}</td>
                            <td align="center">


                                <a href="fafangwuzi_detail.do?id=${map.id}">详情</a>
                                <a href="fafangwuzi_updt.do?id=${map.id}">编辑</a>
                                <a href="fafangwuzi_delete.do?id=${map.id}" onclick="return confirm('确定要删除？')">删除</a>
                                <!--qiatnalijne-->
                            </td>
                        </tr>
                    </c:forEach>
