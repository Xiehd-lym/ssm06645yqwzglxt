...
public class LiuyanbanController extends BaseController
{
    ...
    public String list()
    {
        ...
        List<Liuyanban> list = service.selectPageExample(example , page , 12);   // 获取当前页的行数
        // 将列表写给界面使用
        assign("list" , list);
        assign("orderby" , order);  // 把当前排序结果写进前台
        assign("sort" , sort);      // 把当前排序结果写进前台
        assign("where" , where);    // 把当前条件写给前台
        return "liuyanban_list";   // 使用视图文件：WebRoot\liuyanban_list.jsp
    }
    ...
    public String listliuyanren()
    {
        ...
        assign("sort" , sort);
        assign("where" , where);
        return "liuyanban_list_liuyanren";
    }
