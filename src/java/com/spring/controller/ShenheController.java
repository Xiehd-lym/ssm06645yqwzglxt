...
public class ShenheController extends BaseController
{
    ...
    public String list()
    {
        ...
        Example example = new Example(Shenhe.class); //  创建一个扩展搜索类
        ...
        List<Shenhe> list = service.selectPageExample(example , page , 12);   // 获取当前页的行数
        // 将列表写给界面使用
        assign("list" , list);
        assign("orderby" , order);  // 把当前排序结果写进前台
        assign("sort" , sort);      // 把当前排序结果写进前台
        assign("where" , where);    // 把当前条件写给前台
        return "shenhe_list";   // 使用视图文件：WebRoot\shenhe_list.jsp
    }
    ...
    public String getWhere()
    {
        ...
        // 以下是判断搜索框中是否有输入内容，判断是否前台是否有填写相关条件，符合则写入sql搜索语句
            ...
            where += " AND xingming LIKE '%"+Request.get("xingming")+"%' ";
                ...
                if(!Request.get("shenhemiaoshu").equals("")) {
            where += " AND shenhemiaoshu LIKE '%"+Request.get("shenhemiaoshu")+"%' ";
        }
                if(!Request.get("shenpiren").equals("")) {
            where += " AND shenpiren LIKE '%"+Request.get("shenpiren")+"%' ";
        }
            return where;
    }
     ...
     * 申请人列表
     */
    @RequestMapping("/shenhe_list_shenqingren")
    public String listshenqingren()
    {
        ...
        page = Math.max(1 , page); // 注释同list
            ...
            List<Shenhe> list = service.selectPageExample(example , page , 12);
        
        request.setAttribute("list" , list);
                assign("orderby" , order);
        assign("sort" , sort);
        assign("where" , where);
        return "shenhe_list_shenqingren";
    }
    /**
     * 审批人列表
     */
    @RequestMapping("/shenhe_list_shenpiren")
    public String listshenpiren()
    {
        // 检测是否有登录，没登录则跳转到登录页面
        if(!checkLogin()){
            return showError("尚未登录" , "./login.do");
        ...
        Example example = new Example(Shenhe.class);  //  创建一个扩展搜索类
        Example.Criteria criteria = example.createCriteria();           // 创建一个扩展搜索条件类
        // 初始化一个条件，条件为：审批人=当前登录用户
        String where = " shenpiren='"+session.getAttribute("username")+"' ";
        where += getWhere();
    ...
    @RequestMapping("/shenhe_updt")
    public String updt()
    {
        int id = Request.getInt("id");
        // 获取行数据，并赋值给前台jsp页面
        Shenhe mmm = service.find(id);
     ...
     * @return
     */
    @RequestMapping("/shenheinsert")
    public String insert()
    {
        String tmp="";
        Shenhe post = new Shenhe();  // 创建实体类
        // 设置前台提交上来的数据到实体类中
        ...
        post.setShenhemiaoshu(Request.get("shenhemiaoshu"));

        post.setShenpiren(Request.get("shenpiren"));
        ...
        Query.execute("update shenqingwuzi set zhuangtai='"+request.getParameter("shenhe")+"' where id='"+request.getParameter("shenqingwuziid")+"'");
    ...
    * @return
    */
    @RequestMapping("/shenheupdate")
    public String update()
    {
        // 创建实体类
        Shenhe post = new Shenhe();
        // 将前台表单数据填充到实体类
        if(!Request.get("bianhao").equals(""))
        post.setBianhao(Request.get("bianhao"));
                if(!Request.get("mingcheng").equals(""))
     ...
     *  后台详情
     */
    @RequestMapping("/shenhe_detail")
    public String detail()
    {
        int id = Request.getInt("id");
        Shenhe map = service.find(id);  // 根据前台url 参数中的id获取行数据
        request.setAttribute("map" , map);  // 把数据写到前台
        return "shenhe_detail";  // 详情页面：WebRoot\shenhe_detail.jsp
    }
        /**
    *  删除
    */
    @RequestMapping("/shenhe_delete")
    public String delete()
    {
        ...
        HashMap map = Query.make("shenhe").find(id);
